...
def register_web_v1_routes(app) -> None:
    ...
    def operations_home(request: Request):
        ...
        <div class="card"><p><strong>Web V1 foundation</strong></p>
        <p>This is the permanent operational structure that will become the day-to-day booking system. The screens below are foundations only in this milestone; the booking calendar and working forms come next.</p></div>
        <div class="grid">
          <div class="card"><h2>Customers</h2><p><strong>{counts['customer_records']}</strong> customer record(s)</p><p>Permanent Client Register: customer details, history and returning-customer lookup.</p></div>
          <div class="card"><h2>Enquiries</h2><p><strong>{counts['enquiries']}</strong> enquiry record(s)</p><p>New enquiry → availability search → offer.</p></div>
          ...
          <div class="card"><h2>Setup</h2><p>The proven Element Types, Elements, seasons, Person pricing, occupancy and Add-on rules remain available.</p><p><a class="button" href="/setup">Open Setup</a></p></div>
        </div>'''
