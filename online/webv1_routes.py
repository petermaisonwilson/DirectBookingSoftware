...
def register_web_v1_routes(app) -> None:
    ...
    def operations_home(request: Request):
        ...
        <div class="grid">
          <div class="card"><h2>Availability Calendar</h2><p>See free, booked, closed and temporarily held Elements by date. Change Element Type or dates and select an available Element.</p><p><a class="button" href="/availability/calendar">Open Availability Calendar</a> <a class="button secondary" href="/company/hold-settings">Hold timing</a></p></div>
    ...
    def operations_foundation(request: Request):
        ...
        <div class="card"><h2>Walk-in / unbooked arrival</h2><p>An Arrival is deliberately allowed to begin without an existing Booking. That is the foundation for the evening self check-in flow we will build later: identify availability, collect guest details, create the booking and check the guest in without an operator needing to be present.</p></div>
        <div class="card"><h2>Client isolation</h2><p>Every operational record carries a Client ID. Operators see only their own Client. A Supervisor sees a Client's data only while visibly in Support Mode.</p></div>'''
