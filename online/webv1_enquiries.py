...
def register_enquiry_routes(app) -> None:
    ...
    def enquiry_register(request: Request, q: str = '', status: str = '', source: str = '', arrival_from: str = '', arrival_to: str = '', departure_from: str = '', departure_to: str = ''):
        ...
        status_options = '<option value="">All statuses</option>' + ''.join(f'<option value="{value}" {"selected" if value == status_filter else ""}>{esc(value.title())}</option>' for value in STATUSES)
        result_rows = ''.join(f'''<tr><td><a href="/operations/enquiries/{int(row['id'])}">#{int(row['id'])}</a></td><td>{esc(_customer_name(row))}</td><td>{esc(row['status'].title())}</td>
              <td>{esc(row['arrival_date'] or '—')}</td><td>{esc(row['departure_date'] or '—')}</td><td>{esc(row['element_type'] or '—')}</td><td>{esc(row['element_name'] or '—')}</td>
              <td>{'€%.2f' % float(row['provisional_total']) if row['provisional_total'] is not None else '—'}</td><td>{esc(row['source'] or '—')}</td></tr>''' for row in enquiries) or '<tr><td colspan="9" class="muted">No matching enquiries.</td></tr>'
        body = f'''<h1>Enquiries</h1><p><a href="/operations">← Operations</a></p><div class="card"><form method="get" action="/operations/enquiries"><div class="grid">
          <div><label>Customer search</label><input name="q" value="{esc(search)}" placeholder="Name, email or telephone"></div><div><label>Status</label><select name="status">{status_options}</select></div>
          <div><label>Source</label><input name="source" value="{esc(source_filter)}" placeholder="Phone, website, walk-in..."></div><div><label>Arrival from</label><input type="date" name="arrival_from" value="{esc(arrival_from)}"></div>
          <div><label>Arrival to</label><input type="date" name="arrival_to" value="{esc(arrival_to)}"></div><div><label>Departure from</label><input type="date" name="departure_from" value="{esc(departure_from)}"></div>
          <div><label>Departure to</label><input type="date" name="departure_to" value="{esc(departure_to)}"></div></div><p><button type="submit">Search Enquiries</button> <a class="button secondary" href="/operations/enquiries">Clear</a></p></form></div>
          <div class="card"><p><strong>{len(enquiries)}</strong> matching enquiry/enquiries</p><table><thead><tr><th>No.</th><th>Customer</th><th>Status</th><th>Arrival</th><th>Departure</th><th>Element Type</th><th>Element</th><th>Provisional</th><th>Source</th></tr></thead><tbody>{result_rows}</tbody></table></div>'''
    ...
    def enquiry_detail(enquiry_id: int, request: Request, saved: int = 0):
        ...
        if enquiry is None: return HTMLResponse(layout('Enquiry not found', '<div class="error">Enquiry not found.</div>', context), status_code=404)
        ...
        if request_row is None:
            ...
            people_text = ', '.join(f'{esc(r["name"])} × {int(r["quantity"])}' for r in people) or '—'; addons_text = ', '.join(f'{esc(r["name"])} × {int(r["quantity"])}' for r in addons) or '—'
            total_text = f'€{float(request_row["provisional_total"]):.2f}' if request_row['provisional_total'] is not None else 'Not priced yet'; breakdown = ''
            ...
            request_html = f'''<div class="card"><h2>Requested stay</h2><p><strong>Element Type:</strong> {esc(request_row['element_type'] or '—')}<br><strong>Specific Element:</strong> {esc(request_row['element_name'] or 'Not selected')}<br>
              <strong>People:</strong> {people_text}<br><strong>Add-ons:</strong> {addons_text}<br><strong>Provisional total:</strong> {total_text}</p>{breakdown}
              <p><a class="button" href="/operations/enquiries/{enquiry_id}/edit">Edit / Recalculate Enquiry</a></p><p class="muted">This is still an Enquiry. No Offer has been created and no Booking price has been frozen.</p></div>'''
        body = f'''<h1>Enquiry #{int(enquiry['id'])}</h1><p><a href="/operations/enquiries">← Enquiry Search</a></p>{notice}<div class="grid"><div class="card"><h2>Customer</h2><p><strong>{customer_link}</strong></p>
          <p>Email: {esc(enquiry['email'] or '—')}<br>Telephone: {esc(enquiry['phone'] or '—')}</p></div><div class="card"><h2>Enquiry</h2><p><strong>Status:</strong> {esc(enquiry['status'].title())}<br>
          <strong>Arrival:</strong> {esc(enquiry['arrival_date'] or '—')}<br><strong>Departure:</strong> {esc(enquiry['departure_date'] or '—')}<br><strong>Party size:</strong> {esc(enquiry['party_size'] if enquiry['party_size'] is not None else '—')}<br><strong>Source:</strong> {esc(enquiry['source'] or '—')}</p></div></div>
          <div class="card"><h2>Notes</h2><p>{esc(enquiry['notes'] or '—')}</p></div>{request_html}'''
