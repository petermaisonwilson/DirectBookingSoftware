...
def _rules_page(database, context, year=None, element_type='', element_id=0, submitted=None, message='') -> str:
    ...
    body += f'''</tbody></table><p><button>SAVE CHANGES</button></p></div></form><script>(()=>{{const form=document.getElementById('rules-form');const tracked=[...form.querySelectorAll('input,select')].filter(x=>x.type!=='hidden'&&x.id!=='element-select');const snapshot=()=>JSON.stringify(tracked.map(x=>[x.name,x.type==='checkbox'||x.type==='radio'?x.checked:x.value]));const initial=snapshot();const isDirty=()=>snapshot()!==initial;document.querySelectorAll('[data-dirty-nav]').forEach(a=>a.addEventListener('click',e=>{{if(isDirty()&&!confirm('You have unsaved changes. Leave without saving?'))e.preventDefault();}}));const sel=document.getElementById('element-select');if(sel)sel.addEventListener('change',()=>{{if(isDirty()&&!confirm('You have unsaved changes. Leave without saving?')){{sel.value='{chosen_id}';return;}}window.location='/setup/addon-rules?year={selected}&element_type='+encodeURIComponent('{typ}')+'&element='+sel.value;}});}})();</script>'''
    return layout('Feature / Extra Rules', body, context)
