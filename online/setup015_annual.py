...
def _pricing_page(database, context, year: int | None, submitted: dict[str, str] | None = None, errors: set[str] | None = None, message: str = '') -> str:
    ...
    seasons = rows(database, 'SELECT * FROM setup_seasons WHERE company_id=? AND year=? ORDER BY start_date', (cid, selected))
    elements = rows(database, 'SELECT * FROM setup_elements WHERE company_id=? AND active=1 ORDER BY element_type,name', (cid,))
    season_name = submitted.get('season_name', '') if submitted else ''
    season_start = submitted.get('season_start', '') if submitted else ''
    season_end = submitted.get('season_end', '') if submitted else ''
    ...
    for element in elements:
        ...
        for season in seasons:
            ...
            if submitted and key in submitted:
                ...
                rate_row = one(database, 'SELECT rate FROM setup_element_rates WHERE company_id=? AND year=? AND element_id=? AND season_id=?', (cid, selected, element['id'], season['id']))
                value = '' if rate_row is None else f'{float(rate_row["rate"]):.2f}'
...
def _occupancy_page(database, context, year: int | None, submitted: dict[str, str] | None = None, errors: set[str] | None = None, message: str = '') -> str:
    ...
    body = f'<h1>Occupancy</h1>{setup_nav()}{_error_html(message)}{_year_select(available, selected, "/setup/occupancy")}'
    if selected is None:
    ...
    for element in elements:
        ...
        if submitted:
            total_value = submitted.get(total_key, '')
        else:
            total = one(database, 'SELECT max_total FROM setup_occupancy WHERE company_id=? AND year=? AND element_id=?', (cid, selected, element['id']))
            total_value = '' if total is None else str(total['max_total'])
        body += f'<tr><td>{esc(element["name"])}</td><td><input style="{_style(total_key, errors)}" name="{total_key}" value="{esc(total_value)}"></td>'
        for person in people:
            key = f'p_{element["id"]}_{person["id"]}'
            if submitted:
                value = submitted.get(key, '')
            else:
                limit = one(database, 'SELECT max_count FROM setup_person_limits WHERE company_id=? AND year=? AND element_id=? AND person_type_id=?', (cid, selected, element['id'], person['id']))
                value = '' if limit is None else str(limit['max_count'])
            body += f'<td><input style="{_style(key, errors)}" name="{key}" value="{esc(value)}"></td>'
        body += '</tr>'
...
def _addon_page(database, context, year: int | None, submitted: dict[str, str] | None = None, errors: set[str] | None = None, message: str = '') -> str:
    ...
    for typ in types:
        for addon in addons:
            base = one(database, 'SELECT * FROM setup_type_addons WHERE company_id=? AND year=? AND element_type=? AND addon_id=?', (cid, selected, typ, addon['id']))
            check_key = f'ty_{addon["id"]}_{typ}'
            yes = (check_key in submitted) if submitted else bool(base and base['allowed'])
            chk = 'checked' if yes else ''
            vals = []
            for prefix, col in (('tymin', 'min_qty'), ('tymax', 'max_qty'), ('tyrate', 'rate')):
                ...
                if submitted:
                    ...
                    value = '' if not base or base[col] is None else (f'{float(base[col]):.2f}' if col == 'rate' else str(base[col]))
    ...
    for element in elements:
        for addon in addons:
            base = one(database, 'SELECT * FROM setup_element_addons WHERE company_id=? AND year=? AND element_id=? AND addon_id=?', (cid, selected, element['id'], addon['id']))
            radio_key = f'ov_{element["id"]}_{addon["id"]}'
            state = submitted.get(radio_key, 'I') if submitted else ('I' if not base else base['state'])
            radios = ' '.join(f'<label style="display:inline;font-weight:normal"><input style="width:auto" type="radio" name="{radio_key}" value="{s}" {"checked" if state == s else ""}> {s}</label>' for s in ('I', 'Y', 'N'))
            vals = []
...
def register_annual_routes(app) -> None:
    ...
    def pricing_page(request: Request, year: str = ''):
        ...
        return _pricing_page(database, context, selected_year(database, working_company(context), year))
    ...
    async def season_save(request: Request):
        ...
        try:
            ...
            return HTMLResponse(_pricing_page(database, context, None, message='Select a valid pricing year.'), 400)
        ...
        if errors:
            submitted = {'season_name': name, 'season_start': start_raw, 'season_end': end_raw}
            return HTMLResponse(_pricing_page(database, context, year, submitted, errors, 'Complete the highlighted season fields. The end date cannot be before the start date.'), 400)
        with database.connect() as c:
            try:
                sid = c.execute('INSERT INTO setup_seasons(company_id,year,name,start_date,end_date) VALUES (?,?,?,?,?)', (cid, year, name, start_raw, end_raw)).lastrowid
            except Exception:
                submitted = {'season_name': name, 'season_start': start_raw, 'season_end': end_raw}
                return HTMLResponse(_pricing_page(database, context, year, submitted, {'season_name'}, 'That season name already exists for this year.'), 400)
        audit(database, context, cid, 'SEASON_ADDED', 'season', sid, None, {'year': year, 'name': name})
    ...
    async def pricing_save(request: Request):
        ...
        seasons = rows(database, 'SELECT id FROM setup_seasons WHERE company_id=? AND year=?', (cid, year)); elements = rows(database, 'SELECT id FROM setup_elements WHERE company_id=? AND active=1', (cid,)); values = []; errors = set()
        for element in elements:
            for season in seasons:
    ...
    async def occupancy_save(request: Request):
        ...
        for element in elements:
            key = f't_{element["id"]}'
            try: totals.append((element['id'], valid_whole(data.get(key, ''))))
            except (TypeError, ValueError): errors.add(key)
            for person in people:
                key = f'p_{element["id"]}_{person["id"]}'
                try: limits.append((element['id'], person['id'], valid_whole(data.get(key, ''))))
                except (TypeError, ValueError): errors.add(key)
        if errors: return HTMLResponse(_occupancy_page(database, context, year, data, errors, 'Occupancy values must be whole numbers of zero or more. Zero remains valid.'), 400)
        with database.connect() as c:
