...
def register_calendar_v5_routes(app) -> None:
    ...
    def calendar(request: Request, element_type: str = '', start: str = '', arrival: str = '', departure: str = '', edit_hold: int = 0):
        ...
        <div id="hold-modal" class="hold-modal" hidden><div class="hold-dialog"><h2>Still want to hold these Elements?</h2><p id="hold-names"></p><p>If KEEP is not clicked before the hold expires, everything is released automatically.</p><p><button id="hold-yes" type="button">KEEP</button> <button id="hold-release" type="button" class="secondary">RELEASE</button></p></div></div>
        <style>
