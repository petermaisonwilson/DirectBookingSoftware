...
def _form_page(database, context, customer, values: dict[str, str], *, enquiry_id: int | None = None, errors=None, message='', result=None):
    ...
    body += '</div></div><div class="card"><h2>Add-ons</h2><p class="muted">Client-defined When? choices appear only where configured in Setup.</p><table><thead><tr><th>Add-on</th><th>Setup rule</th><th>Quantity</th><th>When?</th></tr></thead><tbody>'
    for addon in addons:
    ...
    body += f'''</tbody></table><p><a href="/setup/addons/when">Configure Add-on When? choices in Setup</a></p></div><div class="card"><h2>Notes</h2><textarea name="notes" rows="5" style="width:100%;padding:9px;border:1px solid #aeb8c4;border-radius:6px">{esc(values.get('notes',''))}</textarea></div><div class="card"><h2>Provisional price</h2><p><button type="submit" name="action" value="calculate">Calculate provisional price</button> <button type="submit" name="action" value="save">Save Enquiry</button></p></div>'''
    if result:
    ...
    body += f'''<script>(function(){{const elements={json.dumps(element_json)};const rules={json.dumps(rule_map)};const personRates={json.dumps(person_rate_map)};const dayValues={json.dumps(day_values)};const typeSel=document.getElementById('element_type');const elementSel=document.getElementById('element_id');const arrival=document.getElementById('arrival_date');const departure=document.getElementById('departure_date');const originalElement={json.dumps(str(selected_element_id) if selected_element_id else '')};function dates(){{if(!arrival.value||!departure.value)return[];const a=new Date(arrival.value+'T00:00:00Z'),b=new Date(departure.value+'T00:00:00Z');if(!(b>a))return[];const out=[];for(let d=new Date(a);d<b;d.setUTCDate(d.getUTCDate()+1))out.push(d.toISOString().slice(0,10));return out;}}function pretty(s){{const p=s.split('-');return new Date(Date.UTC(+p[0],+p[1]-1,+p[2])).toLocaleDateString(undefined,{{weekday:'short',day:'numeric',month:'short'}});}}function renderDays(aid){{const sel=document.querySelector('.addon-when[data-addon-id="'+aid+'"]');const row=document.getElementById('addon-days-row-'+aid);const box=document.getElementById('addon-days-'+aid);if(!sel||sel.value!=='selected_days'){{row.style.display='none';box.innerHTML='';return;}}const base=document.querySelector('.addon-input[data-addon-id="'+aid+'"]');const saved=dayValues[String(aid)]||{{}};box.innerHTML='<strong>Selected days</strong><div class="grid">'+dates().map(function(d){{const v=(saved[d]!==undefined?saved[d]:(base.value||'0'));return '<div><label>'+pretty(d)+'</label><input type="number" min="0" name="addon_day_'+aid+'_'+d+'" value="'+v+'"></div>';}}).join('')+'</div>';row.style.display='table-row';}}function rebuildElements(){{const wanted=typeSel.value;const old=elementSel.value||originalElement;elementSel.innerHTML='<option value="">-- no specific Element yet --</option>';elements.filter(e=>e.type===wanted).forEach(function(e){{const o=document.createElement('option');o.value=String(e.id);o.textContent=e.name;if(String(e.id)===old)o.selected=true;elementSel.appendChild(o);}});applyRules();}}function applyRules(){{const year=arrival.value?arrival.value.slice(0,4):'';const eid=elementSel.value;const configured=Boolean(year&&eid);document.querySelectorAll('.person-input').forEach(function(input){{input.disabled=!eid;const pid=input.dataset.personId;const rate=((personRates[year]||{{}})[eid]||{{}})[pid];document.getElementById('person-rate-'+pid).textContent=(rate===undefined?'':'€'+Number(rate).toFixed(2));}});document.querySelectorAll('.addon-input').forEach(function(input){{const aid=input.dataset.addonId;const rule=(((rules[year]||{{}})[eid]||{{}})[aid]);const text=document.getElementById('addon-rule-'+aid);if(configured&&rule&&rule.allowed){{input.disabled=false;input.max=String(rule.max);text.textContent=rule.source+'; min '+rule.min+', max '+rule.max+'; €'+Number(rule.rate).toFixed(2);}}else{{input.disabled=true;if(configured&&rule)text.textContent=rule.source;else text.textContent='Choose dates and a specific Element.';}}renderDays(aid);}});}}document.querySelectorAll('.addon-when').forEach(function(s){{s.addEventListener('change',function(){{renderDays(s.dataset.addonId);}});}});typeSel.addEventListener('change',function(){{elementSel.value='';rebuildElements();}});elementSel.addEventListener('change',applyRules);arrival.addEventListener('change',applyRules);departure.addEventListener('change',applyRules);rebuildElements();}})();</script>'''
