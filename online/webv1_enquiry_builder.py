...
def _form_page(database, context, customer, values: dict[str, str], *, enquiry_id: int | None = None, errors=None, message='', result=None):
    ...
    body += '</div></div><div class="card"><h2>Add-ons</h2><p class="muted">Choose Every day for one quantity across the stay, or Selected days to enter quantities only on the dates wanted.</p><table><thead><tr><th>Add-on</th><th>Setup rule</th><th>Quantity</th><th>When?</th></tr></thead><tbody>'
    for addon in addons:
        ...
        body += f'''<tr><td>{esc(addon['name'])}</td><td id="addon-rule-{aid}" class="muted">Choose dates and an Element.</td><td><div class="addon-main-qty" data-addon-id="{aid}"><input class="addon-input" data-addon-id="{aid}" style="{style(key)}" type="number" min="0" name="{key}" value="{esc(values.get(key,'0'))}"></div></td><td><select class="addon-when" data-addon-id="{aid}" name="addon_when_{aid}">{option_html}</select></td></tr><tr class="addon-days-row" id="addon-days-row-{aid}" style="display:none"><td colspan="4"><div id="addon-days-{aid}"></div></td></tr>'''
...
function renderDays(aid){{
 const sel=document.querySelector('.addon-when[data-addon-id="'+aid+'"]');const row=document.getElementById('addon-days-row-'+aid);const box=document.getElementById('addon-days-'+aid);const qtyWrap=document.querySelector('.addon-main-qty[data-addon-id="'+aid+'"]');const base=document.querySelector('.addon-input[data-addon-id="'+aid+'"]');if(!sel){{row.style.display='none';box.innerHTML='';return;}}
 const saved=dayValues[String(aid)]||{{}};const allDates=dates();
 if(sel.value==='selected_days'){{
   qtyWrap.style.display='none';base.disabled=true;
   ...
   qtyWrap.style.display='block';box.innerHTML='';row.style.display='none';
 }}
}}
...
function applyRules(){{const year=arrival.value?arrival.value.slice(0,4):'';const eid=elementSel.value;const configured=Boolean(year&&eid);document.querySelectorAll('.person-input').forEach(function(input){{input.disabled=!eid;const pid=input.dataset.personId;const rate=((personRates[year]||{{}})[eid]||{{}})[pid];document.getElementById('person-rate-'+pid).textContent=(rate===undefined?'':'€'+Number(rate).toFixed(2));}});document.querySelectorAll('.addon-input').forEach(function(input){{const aid=input.dataset.addonId;const rule=(((rules[year]||{{}})[eid]||{{}})[aid]);const text=document.getElementById('addon-rule-'+aid);const when=document.querySelector('.addon-when[data-addon-id="'+aid+'"]');const selected=when&&when.value==='selected_days';if(configured&&rule&&rule.allowed){{input.disabled=selected;input.max=String(rule.max);text.textContent=rule.source+'; min '+rule.min+', max '+rule.max+'; €'+Number(rule.rate).toFixed(2);}}else{{input.disabled=true;if(configured&&rule)text.textContent=rule.source;else text.textContent='Choose dates and a specific Element.';}}renderDays(aid);}});}}
document.querySelectorAll('.addon-when').forEach(function(s){{s.addEventListener('change',function(){{applyRules();}});}});
