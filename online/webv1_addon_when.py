...
def register_addon_when_routes(app) -> None:
    ...
    def addon_when_setup(request: Request, saved: int = 0, year: int = 0, error: str = ''):
        context=context_for(database,request); cid=working_company(context); addons=rows(database,"""SELECT * FROM setup_addons WHERE company_id=? ORDER BY CASE item_kind WHEN 'Feature' THEN 0 WHEN 'Extra' THEN 1 ELSE 2 END, COALESCE(NULLIF(TRIM(feature_group),''),'Other') COLLATE NOCASE, active DESC, name COLLATE NOCASE""",(cid,)); people=rows(database,'SELECT * FROM setup_person_types WHERE company_id=? AND active=1 ORDER BY name COLLATE NOCASE',(cid,)); year_rows=rows(database,'SELECT year FROM setup_years WHERE company_id=? ORDER BY year DESC',(cid,)); available=[int(r['year']) for r in year_rows]; selected_year=year if year in available else (available[0] if available else 0)
        ...
        body=f'''<h1>Feature / Extra Timings &amp; Person Pricing</h1><p><a href="/setup/addons">← Features / Extras</a> &nbsp; <a href="/setup">Setup home</a></p>{notice}{error_html}<div class="card"><p>Keep ordinary extras on <strong>One price for everyone</strong>. For Breakfast-style extras, choose <strong>Price by Person Type</strong>.</p><p class="muted">Items are grouped first by <strong>Feature</strong> or <strong>Extra</strong>, then by their configured Group, to make client editing easier.</p><p class="muted">Person Type short codes are used on compact booking screens. Keep them clear and descriptive, maximum 8 characters.</p><div>{year_links}</div></div>'''
        current_kind=None; current_group=None
        for addon in addons:
            kind=str(addon['item_kind'] or '').strip() or 'Other'; group=str(addon['feature_group'] or '').strip() or 'Other'
            if kind!=current_kind:
                current_kind=kind; current_group=None
                body+=f'<div class="addon-kind-heading"><h2>{esc(kind.upper()+"S" if not kind.upper().endswith("S") else kind.upper())}</h2></div>'
            if group!=current_group:
                current_group=group
                body+=f'<div class="addon-group-heading"><h3>{esc(group)}</h3></div>'
            ...
            body+=f'''<div class="card addon-option-card" data-kind="{esc(kind)}" data-group="{esc(group)}"><h4>{esc(addon['name'])}</h4><p class="muted">Pricing method: {esc(addon['pricing_method'])}</p>{note}<form method="post" action="/setup/addons/when"><input type="hidden" name="csrf" value="{esc(context['csrf_token'])}"><input type="hidden" name="addon_id" value="{aid}"><input type="hidden" name="year" value="{selected_year}"><h5>When?</h5><div class="grid"><div><label><input style="width:auto" type="checkbox" name="every_active" value="1" {'checked' if every and every['active'] else ''}> Enable every-day choice</label><label>Label</label><input name="every_label" value="{esc(every['label'] if every else 'Every day')}"></div><div><label><input style="width:auto" type="checkbox" name="selected_active" value="1" {'checked' if selected and selected['active'] else ''}> Enable selected-days choice</label><label>Label</label><input name="selected_label" value="{esc(selected['label'] if selected else 'Selected days')}"></div></div><h5>Price</h5><label>How is this Add-on priced?</label><select class="addon-pricing-mode" data-addon="{aid}" name="pricing_mode" {disabled}><option value="single" {'selected' if mode=='single' else ''}>One price for everyone</option><option value="person_type" {'selected' if mode=='person_type' else ''}>Price by Person Type</option></select><div class="person-rate-panel" data-addon="{aid}" style="margin-top:12px"><div class="grid">{rate_inputs}</div></div><p><button>Save Add-on options</button></p></form></div>'''
        body+='''<style>.addon-kind-heading{margin:28px 0 8px;padding:10px 14px;background:#eef3f7;border-left:5px solid #6d8196;border-radius:6px}.addon-kind-heading h2{margin:0}.addon-group-heading{margin:16px 0 8px;padding:6px 12px;border-bottom:2px solid #d7e0e8}.addon-group-heading h3{margin:0}.addon-option-card h4{margin-top:0;font-size:1.15rem}.addon-option-card h5{font-size:1rem;margin-bottom:8px}</style><script>(function(){function refresh(s){const p=document.querySelector('.person-rate-panel[data-addon="'+s.dataset.addon+'"]');if(p)p.style.display=s.value==='person_type'?'block':'none';}document.querySelectorAll('.addon-pricing-mode').forEach(function(s){s.addEventListener('change',function(){refresh(s);});refresh(s);});})();</script>'''
        return layout('Feature / Extra Timings & Person Pricing',body,context)
