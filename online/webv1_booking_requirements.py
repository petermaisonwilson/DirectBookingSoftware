...
def initialise_booking_requirements(database):
    """Alembic owns requirement tables/columns; startup only removes orphan snapshots."""
...
def _element_reasons(database,cid,year,element,people,addons):
    ...
    for pid,data in people.items():
        ...
        elif qty>int(limit['max_count']):reasons.append(f'{str(p["name"]) if p else "Person type"} not allowed' if int(limit['max_count'])==0 else f'{str(p["name"]) if p else "Person type"} max {int(limit["max_count"])}')
...
def register_booking_requirement_routes(app)->None:
    ...
    def start(request:Request,edit_hold:int=0):
        context,cid=_session_context(database,request);token=request.cookies.get(COOKIE_NAME,'')
        if edit_hold:_load_hold_requirements_into_working(database,cid,token,edit_hold)
        return HTMLResponse(_requirements_page(database,context,cid,token,edit_hold=edit_hold))
