...
def load_runtime_config() -> RuntimeConfig:
    ...
    if environment not in VALID_ENVIRONMENTS:
        raise RuntimeError("DIRECTBOOKING_ENV must be development, test, or production")
    database_url = os.environ.get("DIRECTBOOKING_DATABASE_URL", "").strip()
    if not database_url:
        legacy_path = os.environ.get("DIRECTBOOKING_DB", "online_data/direct_booking_online_dev.db")
        database_url = _sqlite_url_from_path(legacy_path)
    explicit_seed = os.environ.get("DIRECTBOOKING_SEED_DEMO")
    seed_demo = environment == "development" if explicit_seed is None else explicit_seed.strip() == "1"
    if environment == "production" and seed_demo:
        raise RuntimeError("Demo data is forbidden in production")
    secure_cookies = environment == "production" or os.environ.get("DIRECTBOOKING_SECURE_COOKIES", "0").strip() == "1"
    host = os.environ.get("DIRECTBOOKING_HOST", "127.0.0.1").strip()
    port = int(os.environ.get("DIRECTBOOKING_PORT", "8000"))
    return RuntimeConfig(environment, database_url, seed_demo, secure_cookies, host, port)
