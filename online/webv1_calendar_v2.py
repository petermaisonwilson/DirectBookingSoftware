...
def _bar(label: str, colour: str, columns, *, css='status', href='', title='') -> str:
    ...
    a, b = columns
    text = esc(label); title_attr = f' title="{esc(title)}"' if title else ''
...
def register_calendar_v2_routes(app) -> None:
    ...
    def calendar(request: Request, element_type: str = '', start: str = '', arrival: str = '', departure: str = ''):
        ...
        visible_start = _day(start, _day(arrival, date.today())); visible_end = visible_start + timedelta(days=CALENDAR_DAYS)
        token = request.cookies.get(COOKIE_NAME, '')
        exact = []; exact_message = ''
        if selected_type and arrival and departure:
            try:
                a=date.fromisoformat(arrival); d=date.fromisoformat(departure)
                if d<=a: exact_message='Departure must be after arrival.'
                else:
                    exact=available_elements(database,cid,selected_type,arrival,departure,session_token=token)
                    if not exact: exact_message=f'No {selected_type} Elements are available for {_fmt(arrival)} to {_fmt(departure)}.'
            except ValueError: exact_message='Enter valid arrival and departure dates.'
        ...
        <div class="card"><form method="get" action="/availability/calendar-v2"><div class="grid">
          <div><label>Element Type</label><select name="element_type">{options}</select></div>
          <div><label>Arrival</label><input type="date" name="arrival" value="{esc(arrival)}"></div>
          <div><label>Departure</label><input type="date" name="departure" value="{esc(departure)}"></div>
          <div><label>Calendar starts</label><input type="date" name="start" value="{visible_start.isoformat()}"></div>
        </div><p><button>Check availability</button>
        ...
        for i,d in enumerate(dates):
            header+=f'<div class="cal-date" style="grid-column:{i+2};grid-row:1"><strong>{d.strftime("%d")}</strong><small>{d.strftime("%b")}</small></div>'
        header+='</div>'
        rows_html=''
        ...
        held_status=default_status(database,cid,'HELD'); held_colour=str(held_status['colour']) if held_status else '#FFE39A'; held_name=str(held_status['name']) if held_status else 'Enquiry / Held'
        for element in elements:
            eid=int(element['id']); bookings,enquiries,closures,holds=_records(database,cid,eid,visible_start,visible_end)
            windows={d.year:operating_window(database,cid,d.year) for d in dates}
            cells=''
            for i,d in enumerate(dates):
                window=windows[d.year]; state='available'
                if window is None or d<window[0] or d+timedelta(days=1)>window[1]: state='out'
                elif any(_overlap(r['arrival_date'],r['departure_date'],d) and _blocking_booking(r) for r in bookings): state='booked'
                elif any(_overlap(r['arrival_date'],r['departure_date'],d) for r in enquiries): state='held'
                elif any(_overlap(r['start_date'],r['end_date'],d) for r in closures): state='closed'
                elif any(_overlap(r['arrival_date'],r['departure_date'],d) for r in holds): state='held'
                col=i+2
                if state=='available':
                    href=f'/availability/calendar-v2?element_type={quote_plus(selected_type)}&start={visible_start.isoformat()}&arrival={d.isoformat()}&departure={(d+timedelta(days=1)).isoformat()}'
                    cells+=f'<a class="cal-cell available" style="grid-column:{col};grid-row:1" href="{href}" title="Available {d.strftime("%d/%m/%Y")}"></a>'
                else:
                    cells+=f'<span class="cal-cell {state}" style="grid-column:{col};grid-row:1"></span>'
            bars=''
            for b in bookings:
                if not staff and not _blocking_booking(b): continue
                cols=_cols(b['arrival_date'],b['departure_date'],visible_start,visible_end)
                if staff:
                    customer=(f"{b['first_name']} {b['last_name']}").strip() or 'Customer'; status_name=str(b['workflow_name'] or 'Booked'); colour=str(b['colour'] or '#F3C5C9')
                ...
                else: bars+=_bar('Booked','#F3C5C9',cols,title='Unavailable')
            for e in enquiries:
                cols=_cols(e['arrival_date'],e['departure_date'],visible_start,visible_end)
                if staff:
                    customer=(f"{e['first_name']} {e['last_name']}").strip() or 'Customer'; colour=str(e['colour'] or held_colour); label=f'{customer} · Enquiry #{int(e["enquiry_id"])} · {e["workflow_name"] or held_name}'
                ...
                else: bars+=_bar('Held',held_colour,cols,title='Temporarily unavailable')
            for c in closures:
                label=str(c['reason'] or 'Closed') if staff else 'Closed'; bars+=_bar(label,'#CFD4DA',_cols(c['start_date'],c['end_date'],visible_start,visible_end),css='closed')
            for h in holds:
                own=str(h['session_token'])==token; label='Held by you' if own else (held_name if staff else 'Held'); bars+=_bar(label,held_colour,_cols(h['arrival_date'],h['departure_date'],visible_start,visible_end),css='held')
        ...
        if not rows_html: rows_html='<div class="card"><p>No active Elements exist for this Element Type.</p></div>'
        body+=f'<div class="card"><div class="calendar-scroll"><div class="calendar-grid" style="--days:{CALENDAR_DAYS}">{header}{rows_html}</div></div><p class="muted">Clear cells are available. Click a clear date to begin selecting a stay.</p></div>'

        if arrival and departure:
            body+='<div class="card"><h2>Available for your selected dates</h2>'
            if exact_message: body+=f'<div class="error"><strong>{esc(exact_message)}</strong><br>Try different dates or another Element Type.</div>'
            else:
        ...
        <style>
        .calendar-scroll{{overflow-x:auto;border:1px solid #d6dde5;border-radius:8px}} .calendar-grid{{min-width:calc(170px + (var(--days) * 48px));background:white}}
        .cal-row{{display:grid;grid-template-columns:170px repeat(var(--days),48px);position:relative;min-height:48px;border-bottom:1px solid #e1e6eb}} .cal-head{{min-height:52px;background:#f4f6f8;z-index:8}}
        .cal-name{{padding:10px 8px;position:sticky;left:0;z-index:6;background:white;border-right:1px solid #d6dde5}} .cal-head .cal-name{{background:#f4f6f8}}
        .cal-date{{padding:6px 2px;text-align:center;border-right:1px solid #e5e9ee;font-size:12px}} .cal-date small{{display:block;color:#66717f}}
        .cal-cell{{min-height:48px;border-right:1px solid #eef1f4;display:block;z-index:1}} .cal-cell.available{{background:white}} .cal-cell.out{{background:#eef1f4}} .cal-cell.closed{{background:#e4e7eb}} .cal-cell.held{{background:#fff0c2}} .cal-cell.booked{{background:#f8d7da}}
        ...
        <script>
        const csrf={json.dumps(str(context['csrf_token']))}; const arr={json.dumps(arrival)}; const dep={json.dumps(departure)};
    ...
    def booking_detail(request: Request, booking_id: int):
        ...
        if b is None: return HTMLResponse(layout('Booking','<h1>Booking not found</h1>',context),404)
        ...
        for e in elements: body+=f'<tr><td>{esc(e["element_name"])}</td><td>{_fmt(e["arrival_date"])}</td><td>{_fmt(e["departure_date"])}</td><td>€{float(e["total_amount"]):.2f}</td></tr>'
